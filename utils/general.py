...
#             elif SIoU:  # SIoU (Skew IoU) https://arxiv.org/pdf/2205.12740.pdf
#                 # Step 1: Distance penalty (same as DIoU)
#                 distance_penalty = rho2 / c2

#                 # Step 2: Aspect ratio penalty
#                 arctan1 = torch.atan(w1 / (h1 + eps))
#                 arctan2 = torch.atan(w2 / (h2 + eps))
#                 v = 4 / (math.pi ** 2) * torch.pow(arctan1 - arctan2, 2)
#                 with torch.no_grad():
#                     alpha_s = v / ((1 + eps) - iou + v)

#                 aspect_ratio_penalty = v * alpha_s

#                 # Step 3: Shape penalty (related to aspect ratio but scales based on the box sizes)
#                 beta = 2 * alpha_s
#                 w_diff = torch.abs(w1 - w2)
#                 h_diff = torch.abs(h1 - h2)
#                 shape_penalty = (w_diff / (w1 + eps)) * (h_diff / (h1 + eps))

#                 # Combine penalties
#                 return iou - distance_penalty - aspect_ratio_penalty - shape_penalty
