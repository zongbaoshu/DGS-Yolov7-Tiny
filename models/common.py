...
class GlobalAttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        global_features = self.global_pool(x).view(batch_size, channels)
        
       
        attention = F.relu(self.fc1(global_features))
        attention = self.sigmoid(self.fc2(attention)).view(batch_size, channels, 1, 1) 
...
class DGSConv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, reduction=16, ratio=2):
        super(DGSConv, self).__init__()
       

       
        self.depthwise_separable_conv = DepthwiseSeparableConv(c1, c2, kernel_size=k, stride=s, padding=k//2)

      
        self.ghost_channels = c2 // ratio 
        self.cheap_operation = nn.Conv2d(self.ghost_channels, self.ghost_channels, kernel_size=3, stride=1, padding=1, groups=self.ghost_channels, bias=False)
        ...
        self.attention = nn.Sequential(
            nn.Conv2d(c2, c2 // reduction, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(c2 // reduction, c2, kernel_size=1),
            nn.Sigmoid()
        )

    def forward(self, x):
       
        x_main = self.depthwise_separable_conv(x)

       
        cheap_feature = x_main[:, :self.ghost_channels, :, :]
        ghost_feature = self.cheap_operation(cheap_feature)

     
        expanded_feature = torch.cat([cheap_feature, ghost_feature], dim=1)

     
        x_main = self.act(self.bn(expanded_feature))

     
        attention_weights = self.attention(x_main)
        x_main = x_main * attention_weights
